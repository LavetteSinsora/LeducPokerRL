"""
Belief Confident Trainer — Self-play trainer for the Belief Confident Agent.

Training protocol:
  1. Self-play using the belief confident agent against itself.
  2. TD(0) on post-action state chains (same as BeliefTrainer).
  3. After each game, train the likelihood model using revealed opponent hands.
  4. Confidence naturally varies during training: starts at 0 and increases
     as more games are played in the session (batch).

The key difference from BeliefTrainer is that:
  - The confidence score is threaded through all encodings.
  - Training is session-aware: multiple episodes form a "session" where
    confidence naturally increases, teaching the model to handle varying
    confidence levels.
"""
...
class BeliefConfidentTrainer(BaseTrainer):
    """
    Trainer for BeliefConfidentAgent.

    Combines:
      - TD(0) value learning on post-action state chains with confidence
      - Cross-entropy likelihood model training on revealed opponent hands
      - Session-aware confidence that naturally varies during training
    """

    def __init__(self, agent: BeliefConfidentAgent, learning_rate: float = 1e-4,
        ...
        # Session tracking: the agent's game counter increments across
        # episodes within the same batch, simulating a session.
        self._session_game_count = 0
    ...
    def collect_episode(self) -> Tuple[List[List[torch.Tensor]], List[float],
        ...
        Returns:
            ...
            confidence: the confidence level used during this episode
        """
        self.game.reset()
        chains = [[], []]
        likelihood_data = []

        # Set the confidence for this episode based on session progress
        confidence = min(self._session_game_count, self.agent.CONFIDENCE_CAP) / self.agent.CONFIDENCE_CAP
        self.agent.set_game_count(self._session_game_count)
        ...
        while not self.game.is_finished:
            ...
            # Record post-action state for TD(0) with confidence
        ...
        # Increment session game counter after each completed game
        self._session_game_count += 1

        return chains, rewards, likelihood_data, confidence
    ...
    def update_model(self, batch_data: list) -> float:
        ...
        Args:
            batch_data: List of (chains, rewards, likelihood_data, confidence) tuples.
        ...
        # Reset session counter after each batch update (new "session")
        self._session_game_count = 0
    ...
    def _update_value_network(self, batch_data: list) -> float:
        ...
        for chains, rewards, _, _ in batch_data:
            for p_idx in [0, 1]:
                chain = chains[p_idx]
                if not chain:
                    continue
    ...
    def _update_likelihood_model(self, batch_data: list) -> float:
        ...
        for _, _, likelihood_data, _ in batch_data:
    ...
    def debug_episode(self) -> Dict:
        """Play one episode and record detailed trace with belief + confidence."""
        self.game.reset()
        episode_trace = []
