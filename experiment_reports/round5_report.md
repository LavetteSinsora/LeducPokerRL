...
|----|-------|----------|-----------|------------|------------|---------|-------------|
| E1a | belief_cfr | 40K ep | -0.72 | -1.12 | 0.42 | No | Nash too uniform for Bayes |
| **E1b** | **belief_modulated** | **40K sess** | **+0.315** | **-0.43** | **0.51** | **K, J** | **First competitive belief agent** |
| E2a | control (=E1b) | 40K sess | +0.315 | -0.43 | 0.51 | K, J | Same as E1b |
| E2b | belief_oracle | 40K ep | -0.49 | -1.02 | 0.39 | Q, K | Perfect V can't fix noisy beliefs |
| E2c | belief_confident | 2K sess | -0.21 | -0.32 | 0.39 | No | Best short-training — conf=0 optimal |
| E2d | belief_stable | 40K ep | -0.44 | -0.90 | **0.65** | Q, K | Best beliefs, worst TD chain |
...
**But belief quality is terrible**: belief correctness 0.42, barely above random (0.40 for 2-card uniform). The critical failure: belief shift is **BROKEN** in round 1 (0.000 shift). After observing the opponent's first action, the belief vector does not change at all.

**100% FOLD with J** — the agent folds every Jack hand, never raises, never calls.

**Root Cause**: Nash equilibrium likelihoods are too UNIFORM to provide Bayesian signal. At equilibrium, P(RAISE|J) ≈ P(RAISE|Q) ≈ P(RAISE|K) — observing any action barely helps distinguish hands. This is by design: Nash equilibrium play is informationally opaque. If opponents could infer your hand from your actions, they would exploit you — so equilibrium strategies minimize information leakage. Using Nash as a *likelihood model* is fundamentally self-defeating.
...
**Modulation hurts raw accuracy**: -11.1% drop when modulation is applied. But the gate now shows **differentiation across opponent profiles**: heuristic (0.578) > value_based (0.542) > adaptive_value (0.535). This suggests the gate is learning a coarse opponent classification, even if the modulation itself doesn't improve likelihood accuracy.

**Belief correctness**: 0.514 — best among non-stable agents, genuinely above random.
...
**Key insight**: The 2K-session version (avg -0.30) was dramatically undertrained. The full training reveals that population-based training + Nash modulation + sufficient duration produces a competitive belief agent. The critical factor is training duration — the likelihood model needs extensive opponent exposure (1.2M hands) to become informative enough for useful Bayesian updates.

#### Key Insight
> Training duration is the dominant variable for belief agents. A 20× increase in training (2K → 40K sessions) produces a 0.6 chip/round improvement (+0.315 vs -0.30), larger than any architectural change tested in Axis 2. Belief agents' likelihood models require far more data than standard value networks because they must learn opponent behavior patterns across diverse opponent types.
...
**But belief quality kills performance**: belief correctness 0.385 — actually *below* random. The belief-weighted selection computes V = P(J)*V(me,J) + P(Q)*V(me,Q) + P(K)*V(me,K), but when P(J), P(Q), P(K) are essentially random, this produces a random mixture of correct values rather than the correct expected value.
...
**Dramatically better belief correctness**: 0.648 — far above every other agent (0.39-0.45 range). The stable target approach produces genuinely better beliefs because the value function learns a more consistent mapping from belief to value, which in turn provides better gradient signal for belief learning.
...
### 4. Population Training + Duration = Competitive Belief Agent

E1b (population-trained, 40K sessions) at +0.315 vs E1a (self-play, 40K episodes) at -0.72. Both factors matter: population training provides diverse opponent exposure for the likelihood model, and sufficient duration allows the value network to fully exploit the belief signal. Neither alone is sufficient.
...
2. **Population training + sufficient duration is the winning combination.** E1b combines Nash+modulation likelihood, population training (heuristic/value_based/adaptive_value), and 40K sessions. All three factors contribute — neither population training alone (E1b-2K: -0.30) nor self-play (E1a: -0.72) is sufficient.
