"""
FullModulationDeepAgent — DALI_modulation
==========================================
Architecture:
    V(s, opp) = V_base_deep(s)  [frozen]  +  Δ(s, opp_stats)

  V_base_deep : 15→64→64→64→1 (deeper ValueDeepNetwork), frozen
  Δ           : trainable 22→32→32→1 ModulationHead (same as FullModulationAgent)

Purpose:
  Does modulation help when the base is already deeper (higher capacity)?
  If full_modulation_deep outperforms value_based_deep, opponent stats provide
  signal beyond what a deeper base can learn alone. If not, extra base capacity
  already absorbs the opponent-specific information.

Base checkpoint: DALI_modulation/agents/value_based_deep/outputs/seed_{N}/checkpoint_final.pt
"""
...
from preliminary_experiments.dali_variants.value_based_deep.agent import ValueDeepNetwork
...
class FullModulationDeepAgent(BaseAgent):
    """
    Frozen deep value base + ungated residual modulation head.
    V(s, opp) = V_base_deep(s) + Δ(s, opp_stats)
    """
    ...
    def __init__(
        ...
        # ── frozen deep base ──────────────────────────────────────────────────
        self.base = ValueDeepNetwork(GAME_DIMS, hidden_size=64)
        if base_ckpt and os.path.isfile(base_ckpt):
            self.base.load_state_dict(
                torch.load(base_ckpt, map_location="cpu", weights_only=True)
            )
        ...
        if mod_ckpt and os.path.isfile(mod_ckpt):
    ...
    def _get_action_values(self, obs: Observation, opp_stats: np.ndarray) -> list:
