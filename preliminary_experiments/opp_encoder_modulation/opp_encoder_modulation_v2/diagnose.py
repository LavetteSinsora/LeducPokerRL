"""Architecture-specific diagnostics for opponent-encoder modulation v1."""
...
EXPERIMENT_ID = "opp_encoder_modulation_v2"
    ...
    for _ in range(num_hands):
        ...
        while not session.is_finished:
            ...
            if current_player == 0:
                ...
                # Terminal folds bypass the learned value path, so including them
                # as gate=0 would understate how strongly the model uses modulation.
...
def main():
    parser = argparse.ArgumentParser(description="Diagnose opponent-encoder modulation v1.")
    parser.add_argument(
        "--checkpoint",
        type=Path,
    ...
    parser.add_argument(
        ...
        type=int,
        default=200,
        help="Hands per diagnostic opponent.",
    )
    parser.add_argument(
        "--output",
        type=Path,
    ...
    payload = {
        ...
        "per_opponent": {
            opponent_id: run_diagnostics(agent, opponent_id, args.hands)
            for opponent_id in DEFAULT_OPPONENTS
        },
    }
