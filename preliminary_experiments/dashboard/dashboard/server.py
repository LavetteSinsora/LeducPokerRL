...
from preliminary_experiments.promoted_registry.training_manager import TrainingManager
from dashboard.tournament import TournamentRunner
from engine.leduc_game import Action, LeducGame

BASE_DIR = os.path.dirname(os.path.abspath(__file__))
WEB_DIR = os.path.join(BASE_DIR, 'pages')
...
class LeducAPIHandler(http.server.BaseHTTPRequestHandler):
        ...
        if self.path == '/reset':
            ...
            for i, atype in enumerate(agent_types):
                if atype == 'human':
                    ...
                    model_path = registry.get_checkpoint_path(atype)
                    if model_path and os.path.exists(model_path):
                        print(f"Loading trained {atype} model from {model_path}")
                        agent.load_model(model_path)
                    else:
                        print(f"No promoted checkpoint for {atype}, using initial weights.")
            ...
            success = LeducAPIHandler.tournament_runner.run_async(num_rounds, agent_ids=agent_ids)
            self._set_headers()
            self.wfile.write(json.dumps({"success": success}).encode())

        elif self.path == '/api/tournament/stop':
            success = LeducAPIHandler.tournament_runner.request_stop()
            self._set_headers()
            self.wfile.write(json.dumps({"success": success}).encode())
    ...
    def _run_debug_episode_for(self, agent_id: str):
        ...
        if model_path and os.path.exists(model_path):
            ...
            print(f"No saved model for {agent_id}, using initial weights.")
