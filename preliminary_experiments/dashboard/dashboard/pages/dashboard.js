...
let allAgents = [];
let cachedTrainingStats = { loss: 0, avg_chips_per_round: 0 };
...
async function pollStatus() {
    try {
        ...
        // Cache training stats for panel-aware display
        const lossType = data.loss_type || 'Loss';
        cachedTrainingStats = {
            loss: data.stats.loss,
            avg_chips_per_round: data.stats.avg_chips_per_round,
            lossType: lossType
        };

        // Update loss chart axis label if changed
        if (lossChart && lossChart.options.scales.y.title.text !== lossType) {
            lossChart.options.scales.y.title.text = lossType;
            lossChart.data.datasets[0].label = lossType + ' (raw)';
            lossChart.data.datasets[1].label = lossType + ' (smoothed)';
            lossChart.update('none');
        }
...
function getMatchupStats() {
    const opponentIds = Object.keys(rawMatchupData);
    if (opponentIds.length === 0) return { avgChips: 0, numOpponents: 0 };
    let total = 0;
    opponentIds.forEach(id => {
        const pts = rawMatchupData[id];
        if (pts.length > 0) total += pts[pts.length - 1].y;
    });
    return { avgChips: total / opponentIds.length, numOpponents: opponentIds.length };
}

function updateHeaderStats() {
    const lossLabel = document.getElementById('loss-type-label');
    const lossValue = document.getElementById('current-loss');
    const chipsValue = document.getElementById('current-chips-round');

    if (currentPanel === 1) {
        const ms = getMatchupStats();
        lossLabel.innerText = 'Opponents';
        lossValue.innerText = ms.numOpponents;
        const avg = ms.avgChips;
        chipsValue.innerText = (avg >= 0 ? '+' : '') + avg.toFixed(2);
    } else {
        lossLabel.innerText = cachedTrainingStats.lossType || 'Loss';
        lossValue.innerText = cachedTrainingStats.loss.toFixed(6);
        const avg = cachedTrainingStats.avg_chips_per_round;
        chipsValue.innerText = (avg >= 0 ? '+' : '') + avg.toFixed(2);
    }
}
