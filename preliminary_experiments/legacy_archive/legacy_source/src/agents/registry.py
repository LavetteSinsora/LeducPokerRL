...
def _register_builtin_agents():
    ...
    from .residual_value import ResidualValueAgent
    ...
    from src.training.residual_value_trainer import ResidualValueTrainer, ResidualPopValueTrainer
    ...
    # Modulated Pop Value Agent — existing gate architecture + population training
    registry.register(
        id="modulated_pop_value",
        agent_class=ModulatedValueAgent,
        metadata=AgentMetadata(
            id="modulated_pop_value",
            display_name="Modulated Pop Value AI",
            description="Frozen base + gated modulation with population training (ablation condition B)",
            is_trainable=True,
            requires_model_path=True,
            category="rl",
            trainer_class=ModulatedPopTrainer
        )
    )

    # Residual Value Agent (self-play) — frozen base + delta, no gate
    registry.register(
        id="residual_value",
        agent_class=ResidualValueAgent,
        metadata=AgentMetadata(
            id="residual_value",
            display_name="Residual Value AI",
            description="Frozen base + direct residual correction (no gate), self-play training",
            is_trainable=True,
            requires_model_path=True,
            category="rl",
            trainer_class=ResidualValueTrainer
        )
    )

    # Residual Pop Value Agent (population) — frozen base + delta, no gate, diverse opponents
    registry.register(
        id="residual_pop_value",
        agent_class=ResidualValueAgent,
        metadata=AgentMetadata(
            id="residual_pop_value",
            display_name="Residual Pop Value AI",
            description="Frozen base + direct residual correction (no gate), population training",
            is_trainable=True,
            requires_model_path=True,
            category="rl",
            trainer_class=ResidualPopValueTrainer
        )
    )
