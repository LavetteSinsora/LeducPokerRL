"""
Base trainer interface for PokerRL.

Callback protocol
-----------------
Trainers report progress via an optional ``callback(data: dict)`` function.
Two event types are defined:

    {"type": "batch_update",  "episode": int, "loss": float}
    {"type": "evaluation",    "episode": int, "avg_chips_per_round": float}

TrainingManager records these into its history list for the dashboard charts.
"""
