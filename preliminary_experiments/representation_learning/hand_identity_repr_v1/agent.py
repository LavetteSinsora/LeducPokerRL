"""Hand-identity representation encoder for Leduc Hold'em states.

This module defines the encoder network that maps 15-dim state observations
to 8-dim embeddings. Supervision signal is opponent hand identity (J/Q/K)
rather than terminal reward.

Architecture reuses the ContrastiveEncoder from contrastive_repr_v1.
"""
...
class HandIdentityReprAgent(BaseAgent):
    """Agent that produces hand-identity-supervised state embeddings.

    The encoder maps observations to embeddings trained via triplet loss
    or cross-entropy loss, where the supervision signal is the opponent's
    actual hand (J/Q/K). This is a representation learning agent, not a
    policy agent — action selection is random during training data collection.
    ...
    Args:
        ...
        use_classification_head: whether to include CE loss head
    """
