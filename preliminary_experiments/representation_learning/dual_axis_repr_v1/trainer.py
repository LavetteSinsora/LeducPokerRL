"""Trainer for dual-axis contrastive representation learning.
...
in a replay buffer, and trains the encoder using DualAxisContrastiveLoss
with VICRegVarianceLoss as an anti-collapse regularizer.

The key difference from prior experiments: each replay buffer item carries
BOTH a terminal reward AND an opponent hand label. Positive/negative pairs
are constructed jointly from both signals.
"""
...
from experiments.representation_learning.dual_axis_repr_v1.losses import (
    DualAxisContrastiveLoss,
    VICRegVarianceLoss,
)
...
class DualAxisReplayBuffer:
    ...
    def sample(self, n: int) -> Tuple[torch.Tensor, torch.Tensor, torch.Tensor]:
        ...
        Returns:
            (states [N, 15], rewards [N], hand_labels [N])
        """
...
class DualAxisReprTrainer(BaseTrainer):
    """Trains DualAxisReprAgent using joint reward+hand contrastive loss.
    ...
    Args:
        ...
        reward_thresh: |ΔR| < this = reward-similar (positive pair condition)
        reward_margin: |ΔR| > this = reward-dissimilar (negative pair condition)
        lambda_var: VICReg variance weight
        episodes_per_step: episodes collected per training step
    """

    def __init__(self, agent: DualAxisReprAgent,
                 data_agent_path: str = 'agents/value_based/checkpoint.pt',
                 learning_rate: float = 1e-4,
                 batch_size: int = 256,
                 buffer_capacity: int = 5000,
                 temperature: float = 0.5,
                 reward_thresh: float = 0.5,
        ...
        # Loss functions
        self.contrastive_loss_fn = DualAxisContrastiveLoss(
            temperature=temperature,
            reward_thresh=reward_thresh,
            reward_margin=reward_margin,
        )
        self.variance_loss_fn = VICRegVarianceLoss(target_std=1.0)

        # Optimizer over encoder parameters
        self.optimizer = optim.Adam(
            list(agent.encoder.parameters()), lr=learning_rate
        )

        # Logging
        self.loss_history: List[float] = []
        self.contrastive_loss_history: List[float] = []
        self.variance_loss_history: List[float] = []
    ...
    def collect_episode(self) -> List[Tuple[torch.Tensor, float, int]]:
        """Play one self-play episode and return (state_enc, reward, opp_hand) triples.

        For each state visited by player P:
          - state_enc: encoded observation from player P's perspective
          - reward: terminal chip outcome for player P
          - opp_hand_label: opponent's card label (J=0, Q=1, K=2)
        """
        self.game.reset()
        samples = []

        # Collect (state_encoding, player_idx) pairs during play
        step_records = []
        ...
        while not self.game.is_finished:
            ...
            # Opponent's hand is the supervision signal — directly accessible
            ...
            if opponent_label is None:
                # Shouldn't happen in normal play; skip this step
            ...
            step_records.append((encoded, current_player, opponent_label))
        ...
        # Annotate with terminal rewards
        for encoded, player_idx, opp_label in step_records:
            samples.append((encoded, float(rewards[player_idx]), opp_label))
    ...
    def _compute_loss(self) -> float:
        """Compute total loss from replay buffer samples."""
        ...
        # Dual-axis contrastive loss
        contrastive_loss = self.contrastive_loss_fn(z, rewards, hand_labels)

        # VICReg variance loss (anti-collapse)
        variance_loss = self.variance_loss_fn(z)

        total_loss = contrastive_loss + self.lambda_var * variance_loss
        ...
        self.contrastive_loss_history.append(contrastive_loss.item())
        self.variance_loss_history.append(variance_loss.item())
    ...
    def update_params(self, params: Dict):
        ...
        if 'temperature' in params:
            self.contrastive_loss_fn.temperature = params['temperature']
    ...
    def train(self, num_episodes: int, save_path: str = None, callback=None,
        ...
        for step in range(num_steps):
            ...
            if step < 3 or (step + 1) % max(1, num_steps // 20) == 0:
                c_loss = (self.contrastive_loss_history[-1]
                          if self.contrastive_loss_history else 0.0)
                v_loss = (self.variance_loss_history[-1]
                          if self.variance_loss_history else 0.0)
                print(f"Step {step+1}/{num_steps} (ep {episode}), "
                      f"Total: {loss:.5f}  "
                      f"[Contrast: {c_loss:.5f}  VICReg: {v_loss:.5f}]  "
    ...
    def run_linear_probe(self) -> float:
        ...
        states, rewards, labels = self.replay_buffer.get_all()
