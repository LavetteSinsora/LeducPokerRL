"""Trainer for dual-axis hybrid representation learning (v3).
...
buffer, and trains the encoder using the hybrid loss:

    L_total = L_L1_reward + lambda_hand * L_SupCon(hand) + lambda_var * L_VICReg

Key difference from v2: The reward axis uses L1 soft-distance correlation
(continuous metric loss) instead of SupCon over discrete reward bins.
This preserves the continuous metric ordering that SupCon loses.
"""
...
from experiments.representation_learning.dual_axis_repr_v3.losses import (
    SoftDistanceL1Loss,
    SupConLoss,
    VICRegVarianceLoss,
)
...
class DualAxisV3Trainer(BaseTrainer):
    """Trains DualAxisV3Agent using hybrid L1 soft-distance + SupCon hand loss.
    ...
    def __init__(self, agent: DualAxisV3Agent,
        ...
        self.game = LeducGame()
        self.replay_buffer = DualAxisV3ReplayBuffer(capacity=buffer_capacity)
    ...
    def _compute_loss(self) -> Tuple[float, float, float, float]:
        """Compute hybrid L1 + SupCon(hand) + VICReg loss from replay buffer.
    ...
    def train(self, num_episodes: int, save_path: str = None, callback=None,
              start_episode: int = 0):
        """Training loop with replay-buffer-based hybrid loss updates.
        ...
        for step in range(num_steps):
            ...
            # Hybrid loss update from buffer
    ...
    def run_diagnostics(self) -> dict:
        ...
        # Discretize rewards into 5 bins (same thresholds as v2)
