"""Loss functions for dual-axis EMA-normalized representation learning (v4).
...
- SupConLoss: Supervised Contrastive Loss for opponent hand identity (from dual_axis_repr_v2)
- VICRegVarianceLoss: Variance regularization to prevent dimensional collapse
- EMALossNormalizer: Normalizes losses to unit scale before combining

Total loss:
    L_total = (L_reward / ema_reward) + lambda_hand * (L_hand / ema_hand) + lambda_var * L_var

Key difference from v3: losses are normalized by their EMA magnitudes before combining,
fixing the 2,600x scale imbalance that caused reward Spearman rho to collapse to 0.083.
"""
...
class EMALossNormalizer:
    """
    Tracks exponential moving average of loss magnitudes.
    Used to normalize losses to unit scale before combining.

    Usage:
        normalizer = EMALossNormalizer(alpha=0.99, eps=1e-8)
        L_total = normalizer.normalize('reward', L_reward)
                + lambda_hand * normalizer.normalize('hand', L_hand)
                + lambda_var * L_var  # VICReg not normalized (already small scale)

    Args:
        alpha: EMA smoothing factor (default 0.99, slow decay = long memory)
    ...
    def __init__(self, alpha: float = 0.99, eps: float = 1e-8):
        self.alpha = alpha
        self.eps = eps
        self.ema = {}   # key -> running EMA value
        self.step = {}  # key -> update count (for bias correction)

    def normalize(self, key: str, loss: torch.Tensor) -> torch.Tensor:
        """
        Normalize loss by its EMA magnitude.
        Updates EMA with current loss value (detached).
        Returns loss / ema (gradient flows through loss, not ema).

        Args:
            key: identifier string for this loss term (e.g., 'reward', 'hand')
            loss: scalar loss tensor

        Returns:
            normalized loss tensor (gradient-connected through original loss)
        """
        val = loss.item()
        if key not in self.ema:
            ...
            self.ema[key] = self.alpha * self.ema[key] + (1 - self.alpha) * val
            self.step[key] += 1
        # Bias-corrected EMA
        bias_corrected = self.ema[key] / (1 - self.alpha ** self.step[key])
        return loss / (abs(bias_corrected) + self.eps)

    def get_ema(self, key: str) -> float:
        """Return the current (bias-corrected) EMA for a given key."""
        if key not in self.ema:
            return 0.0
        return self.ema[key] / (1 - self.alpha ** self.step[key])
