"""Dual-axis EMA-normalized (v4) representation encoder for Leduc Hold'em states.

This module defines the encoder network that maps 15-dim state observations
to 8-dim embeddings trained via:
  - L1 soft-distance correlation loss (continuous reward metric structure)
  - SupCon loss over opponent hand identity labels
  - EMA normalization to equalize gradient contributions from both losses

Architecture and observation encoding are identical to contrastive_repr_v1,
dual_axis_repr_v2, and dual_axis_repr_v3.
"""
...
class DualAxisV4Agent(BaseAgent):
    """Agent that produces dual-axis EMA-normalized state embeddings.

    Trained via:
      - L_reward: L1 soft-distance correlation (continuous metric structure)
      - L_hand:   SupCon with opponent hand identity labels
      - EMA normalization: equalizes gradient contribution of both losses
