"""
opp_stats_modulation_v2 — Variant B: State-Conditioned Gate Training
=====================================================================
Architecture:  V(s, opp) = V_base(s) [frozen]
                          + g(s, opp_stats) × Δ(s, opp_stats)

  g       : GateNet (22→16→16→1→sigmoid), state+stats input
  Δ       : ModNet  (22→32→32→1), state+stats input

TD target: total value (not residual), so gradients flow through both gate
and mod networks naturally.

  Terminal    : V_target = r
  Non-terminal: V_target = V(s_{t+1})  [detached: no grad through target]
  Loss        = MSE(V(s_t), V_target)

Training recipe: same weighted pool as Variant A (cfr×3, heuristic×3).
...
EXPERIMENT_ID    = "opp_stats_modulation_v2_variant_b_state_gated"
SESSION_LENGTH   = 100
PRIOR_STRENGTH   = 20.0
...
def td_update(agent: StateGatedModAgent, optimizer, criterion, batch_data):
    """
...
def train(out_dir: str, num_episodes: int, smoke: bool):
    ...
    # Jointly optimise gate_net and mod_net parameters
    ...
    config = {
        ...
        "variant":          "variant_b_state_gated",
        "architecture": (
            "StateGatedModAgent("
            "base=frozen, "
            "gate=22→16→16→1→sigmoid, "
            "mod=22→32→32→1, "
            "near-zero-output-init)"
        ),
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train Variant B — State-Conditioned Gate")
    ...
    out_dir  = os.path.join(HERE, "..", "outputs", "variant_b_state_gated")
