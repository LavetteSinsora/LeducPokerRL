"""
AlphaZero v2 — Fixed Opponent Training.

Key differences from v1:
  - Player 1 uses a frozen agent from {value_based, cfr, heuristic}, randomly
    selected each episode. Only Player 0's decisions generate training signal.
  - Reduced architecture: d_model=4, state_hidden=(8,16), belief_hidden=(16,16),
    q_hidden=(64,64).

Usage:
    python experiments/alphazero_v2/train_v2.py
    python experiments/alphazero_v2/train_v2.py --smoke
    python experiments/alphazero_v2/train_v2.py --resume experiments/alphazero_v2/outputs/checkpoint.pt
"""
...
from preliminary_experiments.alphazero.rollout import pimc_search, _step_game
...
def main():
    ...
    # ── Config ────────────────────────────────────────────────────────────────
    config = AZConfig(
        d_model=V2_CONFIG.d_model,
        state_hidden=V2_CONFIG.state_hidden,
        belief_hidden=V2_CONFIG.belief_hidden,
        q_hidden=V2_CONFIG.q_hidden,
        k_rollouts=V2_CONFIG.k_rollouts,
        temperature=V2_CONFIG.temperature,
    ...
    # ── Train ─────────────────────────────────────────────────────────────────
    print(f"\nStarting alphazero_v2 training: {args.episodes} episodes")
