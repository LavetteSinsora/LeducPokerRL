"""
paper — full_modulation training
============================================
Architecture:  V(s, opp) = V_base(s) [frozen] + Δ(s, opp_stats)
               22 → 32 → 32 → 1 modulation head, near-zero output init.

Training recipe:
  - CFR + heuristic oversampled 3×
  - 300K episodes
  - No periodic eval during training
  - Checkpoint every 10K episodes
  - Per-episode JSONL logging

Usage:
  python -m paper.agents.full_modulation.train           # full run
  python -m paper.agents.full_modulation.train --smoke   # 500 episodes
...
def td_update(agent: FullModulationAgent, optimizer, criterion, batch_data):
    """
...
def train(out_dir: str, num_episodes: int, smoke: bool, seed: int):
    ...
    config = {
        "agent":               "FullModulationAgent",
        "architecture":        "V_base(frozen, 15→64→64→1) + ModHead(22→32→32→1)",
    ...
    print(f"\n{'='*65}")
    print(f"  paper — full_modulation  seed={seed}")
    ...
    # Track latest batch stats to report on the episode that triggered update
    last_loss      = None
    last_grad_norm = None
    ...
    for ep in range(1, num_episodes + 1):
        ...
        # Confidence = tracker.get_features()[6]
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(
        description="Train FullModulationAgent (paper)")
